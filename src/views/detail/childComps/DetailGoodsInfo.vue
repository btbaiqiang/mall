<template>
  <div id="detail_goods_info" v-if="Object.keys(detailInfo).length != 0">
    ...
    <div class="goods_info-images" v-for="(item, index) in detailInfo.detailImage" :key="index">
      <div class="goods_info_key">
        {{ item.key }}
      </div>
        <div class="goods_info_imgs" v-for="image in item.list" :key="image.index">
        <img :src="image" alt="" @load="loadImg">
      </div>
    </div>
  </div>
</template>
